...
class CallbackRequestHandler(private val _config: mavenrelatedAuthenticatorPluginConfig): AuthenticatorRequestHandler<CallbackRequestModel>
{
    ...
    private fun handleError(requestModel: CallbackRequestModel)
    {
        if (requestModel.error != null)
        {
            ...
            {
                _logger.debug("Got an error from mavenrelated: {} - {}", requestModel.error, requestModel.errorDescription)

                throw _exceptionFactory.redirectException(
                        _authenticatorInformationProvider.authenticationBaseUri.toASCIIString())
            }

            _logger.warn("Got an error from mavenrelated: {} - {}", requestModel.error, requestModel.errorDescription)

            throw _exceptionFactory.externalServiceException("Login with mavenrelated failed")
        }
    }
